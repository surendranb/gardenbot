...
<script>
    ...
    const CSV_METRICS = GITHUB_RAW + "data/metrics.csv" + CACHE_BUST;
    const CSV_TELEMETRY = GITHUB_RAW + "data/telemetry.csv" + CACHE_BUST;
    const CSV_WEATHER = GITHUB_RAW + "data/weather.csv" + CACHE_BUST;
    ...
    async function refresh() {
        try {
            ...
            const metrics = [
                ...
                { id: 'vpd', key: 'vpd', min: -1.0, precision: 2, source: met }
            ];

            let hardwareWarning = false;

            metrics.forEach(m => {
                const sourceData = m.source || tel;
                const res = findLastValid(sourceData, m.key, m.min);
                const el = document.getElementById(`val-${m.id}`);
                const staleEl = document.getElementById(`stale-${m.id}`);
                
                if (res.value !== null && el) {
            ...
            // Ledger History
            try {
                const res = await fetch(GITHUB_RAW + "logs/warden_history.jsonl?t=" + Date.now());
                if (res.ok) {
                    ...
                    for (let i = lines.length - 1; i >= 0; i--) {
                        ...
                        try {
                            ...
                            html += `
                            <div style="margin-bottom: 20px; border: 1px solid ${isLatest ? '#4ade80' : '#334155'}; border-radius: 8px; padding: 15px; background: ${isLatest ? 'rgba(74, 222, 128, 0.05)' : '#0f172a'}; opacity: ${isLatest ? '1' : '0.6'}; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='${isLatest ? '1' : '0.6'}'">
                                <div style="font-size: 0.75rem; color: ${isLatest ? '#4ade80' : '#94a3b8'}; margin-bottom: 10px; font-weight: bold; display: flex; justify-content: space-between; border-bottom: 1px solid #334155; padding-bottom: 8px;">
                                    <span>${isLatest ? '🟢 LATEST AUDIT' : '⏳ HISTORICAL AUDIT'}</span>
                                    <span>${entry.timestamp}</span>
                                </div>
                                <div style="font-size: 0.85rem; padding-top: 5px;">
                                    ${marked.parse(entry.report)}
                                </div>
                            </div>`;
                        } catch(e) {
                    ...
                    document.getElementById('warden-log-output').innerHTML = html || 'No history available.';
                    // Remove default container styles so cards sit flush
                    const container = document.getElementById('warden-log-output');
    ...
    function drawVitality(data) {
        ...
        window.vChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Mexican Mint (%)', data: data.map(d => parseFloat(d.p2_pct) || null), borderColor: PALETTE.p2 }
                ]
            },
